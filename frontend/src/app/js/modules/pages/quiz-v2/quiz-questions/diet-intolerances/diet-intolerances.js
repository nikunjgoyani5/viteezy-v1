...
angular.module("app.pages.quiz-v2.diet-intolerances", [])
  .config(["$stateProvider", function ($stateProvider) {
    $stateProvider
      .state("diet-intolerances", {
        url: "/quiz-v2/diet-intolerances",
        controller: "DietIntolerancesController",
        controllerAs: "vm",
        bindToController: true,
        templateUrl: "app/js/modules/pages/quiz-v2/quiz-questions/diet-intolerances/diet-intolerances.html",
        role: "anonymous",
        type: "question"
      });
  }])

  .controller("DietIntolerancesController", ["Quiz", "UserManager", "$state", "toast", "QuizAnimation", function (Quiz, UserManager, $state, toast, QuizAnimation) {
    this.questionConfig = {
      nextQuizState: "thirty-minutes-of-sun",
      apiUrlKey: "diet-intolerances",
      apiAnswerValueKey: "dietIntoleranceId",
      currentProgress: 82
    };
    ...
    const addSelectedAnswer = (answer) => {
      Quiz.submitQuestionAnswer({
        ...
        .then((response) => {
          if (response.dietIntoleranceId === 4) {
