...
class TroubleFallingAsleepAnswerServiceImplTest {
    ...
    private static final TroubleFallingAsleepAnswer SAMPLE_ENTITY = new TroubleFallingAsleepAnswer(ID, QUIZ_ID, ALLERGY_ID, LocalDateTime.now(), LocalDateTime.now());
    ...
    private final TroubleFallingAsleepAnswerRepository repository = Mockito.mock(TroubleFallingAsleepAnswerRepository.class);
    private final QuizService quizService = Mockito.mock(QuizService.class);
    private TroubleFallingAsleepAnswerService service;

    @BeforeEach
    void setUp() {
        Mockito.reset(repository);
        service = new TroubleFallingAsleepAnswerServiceImpl(repository, quizService);
    }
    ...
    void find() {
        ...
        Either<Throwable, Optional<TroubleFallingAsleepAnswer>> either = service.find(ID);
    ...
    void findByQuizExternalReference() {
        ...
        Either<Throwable, Optional<TroubleFallingAsleepAnswer>> either = service.find(QUIZ_EXTERNAL_REFERENCE);
        Assertions.assertTrue(either.isRight());
        Assertions.assertTrue(either.get().isPresent());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get().get());
    ...
    void save() {
        ...
        Either<Throwable, TroubleFallingAsleepAnswer> either = service.save(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
    ...
    void update() {
        ...
        Either<Throwable, TroubleFallingAsleepAnswer> either = service.update(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get());
