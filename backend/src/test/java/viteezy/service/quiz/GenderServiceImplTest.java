...
class GenderServiceImplTest {
    ...
    private static final Gender SAMPLE_ENTITY = new Gender(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());

    private final GenderRepository repository = Mockito.mock(GenderRepository.class);
    private GenderServiceImpl service;

    @BeforeEach
    void setUp() {
        Mockito.reset(repository);
        service = new GenderServiceImpl(repository);
    }
    ...
    void find() {
        ...
        Either<Throwable, Optional<Gender>> either = service.find(ID);
    ...
    void findAllOk() {
        ...
        Either<Throwable, List<Gender>> either = service.findAll();
        Assertions.assertTrue(either.isRight());
    ...
    void save() {
        ...
        Either<Throwable, Gender> either = service.save(SAMPLE_ENTITY);
        Assertions.assertTrue(either.isRight());
