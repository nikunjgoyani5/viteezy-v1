...
class HealthComplaintsAnswerServiceImplTest {
    ...
    private static final HealthComplaintsAnswer SAMPLE_ENTITY = new HealthComplaintsAnswer(ID, QUIZ_ID, ALLERGY_ID, LocalDateTime.now(), LocalDateTime.now());
    ...
    private final HealthComplaintsAnswerRepository repository = Mockito.mock(HealthComplaintsAnswerRepository.class);
    private final QuizService quizService = Mockito.mock(QuizService.class);
    private HealthComplaintsAnswerService service;

    @BeforeEach
    void setUp() {
        Mockito.reset(repository);
        service = new HealthComplaintsAnswerServiceImpl(repository, quizService);
    }
    ...
    void find() {
        ...
        Either<Throwable, Optional<HealthComplaintsAnswer>> either = service.find(ID);
    ...
    void findByQuizExternalReference() {
        ...
        Either<Throwable, Optional<HealthComplaintsAnswer>> either = service.find(QUIZ_EXTERNAL_REFERENCE);
        Assertions.assertTrue(either.isRight());
        Assertions.assertTrue(either.get().isPresent());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get().get());
    ...
    void save() {
        ...
        Either<Throwable, HealthComplaintsAnswer> either = service.save(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
    ...
    void update() {
        ...
        Either<Throwable, HealthComplaintsAnswer> either = service.update(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get());
