...
class PresentAtCrowdedPlacesAnswerServiceImplTest {
    ...
    private static final PresentAtCrowdedPlacesAnswer SAMPLE_ENTITY = new PresentAtCrowdedPlacesAnswer(ID, QUIZ_ID, ALLERGY_ID, LocalDateTime.now(), LocalDateTime.now());
    ...
    private final PresentAtCrowdedPlacesAnswerRepository repository = Mockito.mock(PresentAtCrowdedPlacesAnswerRepository.class);
    private final QuizService quizService = Mockito.mock(QuizService.class);
    private PresentAtCrowdedPlacesAnswerService service;

    @BeforeEach
    void setUp() {
        Mockito.reset(repository);
        service = new PresentAtCrowdedPlacesAnswerServiceImpl(repository, quizService);
    }
    ...
    void find() {
        ...
        Either<Throwable, Optional<PresentAtCrowdedPlacesAnswer>> either = service.find(ID);
    ...
    void findByQuizExternalReference() {
        ...
        Either<Throwable, Optional<PresentAtCrowdedPlacesAnswer>> either = service.find(QUIZ_EXTERNAL_REFERENCE);
        Assertions.assertTrue(either.isRight());
        Assertions.assertTrue(either.get().isPresent());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get().get());
    ...
    void save() {
        ...
        Either<Throwable, PresentAtCrowdedPlacesAnswer> either = service.save(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
    ...
    void update() {
        ...
        Either<Throwable, PresentAtCrowdedPlacesAnswer> either = service.update(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get());
