...
class BirthHealthAnswerServiceImplTest {
    ...
    private static final BirthHealthAnswer SAMPLE_ENTITY = new BirthHealthAnswer(ID, QUIZ_ID, ALLERGY_ID, LocalDateTime.now(), LocalDateTime.now());
    ...
    private final BirthHealthAnswerRepository repository = Mockito.mock(BirthHealthAnswerRepository.class);
    private final QuizService quizService = Mockito.mock(QuizService.class);
    private BirthHealthAnswerService service;

    @BeforeEach
    void setUp() {
        Mockito.reset(repository);
        service = new BirthHealthAnswerServiceImpl(repository, quizService);
    }
    ...
    void find() {
        ...
        Either<Throwable, Optional<BirthHealthAnswer>> either = service.find(ID);
    ...
    void findByQuizExternalReference() {
        ...
        Either<Throwable, Optional<BirthHealthAnswer>> either = service.find(QUIZ_EXTERNAL_REFERENCE);
        Assertions.assertTrue(either.isRight());
        Assertions.assertTrue(either.get().isPresent());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get().get());
    ...
    void save() {
        ...
        Either<Throwable, BirthHealthAnswer> either = service.save(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
    ...
    void update() {
        ...
        Either<Throwable, BirthHealthAnswer> either = service.update(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get());
