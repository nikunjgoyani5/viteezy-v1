...
class ChildrenWishAnswerServiceImplTest {
    ...
    private static final ChildrenWishAnswer SAMPLE_ENTITY = new ChildrenWishAnswer(ID, QUIZ_ID, ALLERGY_ID, LocalDateTime.now(), LocalDateTime.now());
    ...
    private final ChildrenWishAnswerRepository repository = Mockito.mock(ChildrenWishAnswerRepository.class);
    private final QuizService quizService = Mockito.mock(QuizService.class);
    private ChildrenWishAnswerService service;

    @BeforeEach
    void setUp() {
        Mockito.reset(repository);
        service = new ChildrenWishAnswerServiceImpl(repository, quizService);
    }
    ...
    void find() {
        ...
        Either<Throwable, Optional<ChildrenWishAnswer>> either = service.find(ID);
    ...
    void findByQuizExternalReference() {
        ...
        Either<Throwable, Optional<ChildrenWishAnswer>> either = service.find(QUIZ_EXTERNAL_REFERENCE);
        Assertions.assertTrue(either.isRight());
        Assertions.assertTrue(either.get().isPresent());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get().get());
    ...
    void save() {
        ...
        Either<Throwable, ChildrenWishAnswer> either = service.save(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
    ...
    void update() {
        ...
        Either<Throwable, ChildrenWishAnswer> either = service.update(CATEGORIZED_ANSWER);
        Assertions.assertTrue(either.isRight());
        Assertions.assertEquals(SAMPLE_ENTITY, either.get());
