...
import viteezy.controller.dto.quiz.HairTypeGetResponse;
import viteezy.controller.dto.quiz.HairTypePostRequest;
import viteezy.domain.quiz.HairType;
import viteezy.service.quiz.HairTypeService;
...
class HairTypeControllerTest {
    ...
    private static final HairType SAMPLE_ENTITY = new HairType(ID, NAME, CODE, true, SUBTITLE, LocalDateTime.now(), LocalDateTime.now());
    private static final HairTypePostRequest SAMPLE_ENTITY_DTO = new HairTypePostRequest(NAME, CODE, true, SUBTITLE);

    private final HairTypeService service = Mockito.mock(HairTypeService.class);
    private HairTypeController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new HairTypeController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((HairTypeGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<HairTypeGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<HairTypeGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<HairType>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((HairType) response.getEntity()).getName());
