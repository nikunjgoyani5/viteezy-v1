...
class DailySixAlcoholicDrinksControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final DailySixAlcoholicDrinks SAMPLE_ENTITY = new DailySixAlcoholicDrinks(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final DailySixAlcoholicDrinksPostRequest SAMPLE_ENTITY_DTO = new DailySixAlcoholicDrinksPostRequest(NAME, CODE, true);

    private final DailySixAlcoholicDrinksService service = Mockito.mock(DailySixAlcoholicDrinksService.class);
    private DailySixAlcoholicDrinksController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new DailySixAlcoholicDrinksController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((DailySixAlcoholicDrinksGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<DailySixAlcoholicDrinksGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<DailySixAlcoholicDrinksGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<DailySixAlcoholicDrinks>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((DailySixAlcoholicDrinks) response.getEntity()).getName());
