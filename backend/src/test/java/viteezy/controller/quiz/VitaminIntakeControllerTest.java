...
class VitaminIntakeControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final VitaminIntake SAMPLE_ENTITY = new VitaminIntake(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final VitaminIntakePostRequest SAMPLE_ENTITY_DTO = new VitaminIntakePostRequest(NAME, CODE, true);

    private final VitaminIntakeService service = Mockito.mock(VitaminIntakeService.class);
    private VitaminIntakeController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new VitaminIntakeController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((VitaminIntakeGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<VitaminIntakeGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<VitaminIntakeGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<VitaminIntake>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((VitaminIntake) response.getEntity()).getName());
