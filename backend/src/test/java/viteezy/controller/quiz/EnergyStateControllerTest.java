...
class EnergyStateControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final EnergyState SAMPLE_ENTITY = new EnergyState(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final EnergyStatePostRequest SAMPLE_ENTITY_DTO = new EnergyStatePostRequest(NAME, CODE, true);

    private final EnergyStateService service = Mockito.mock(EnergyStateService.class);
    private EnergyStateController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new EnergyStateController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((EnergyStateGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<EnergyStateGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<EnergyStateGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<EnergyState>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((EnergyState) response.getEntity()).getName());
