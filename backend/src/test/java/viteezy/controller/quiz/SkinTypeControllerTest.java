...
import viteezy.controller.dto.quiz.SkinTypeGetResponse;
import viteezy.controller.dto.quiz.SkinTypePostRequest;
import viteezy.domain.quiz.SkinType;
import viteezy.service.quiz.SkinTypeService;
...
class SkinTypeControllerTest {
    ...
    private static final SkinType SAMPLE_ENTITY = new SkinType(ID, NAME, CODE, true, SUBTITLE, LocalDateTime.now(), LocalDateTime.now());
    private static final SkinTypePostRequest SAMPLE_ENTITY_DTO = new SkinTypePostRequest(NAME, CODE, true, SUBTITLE);

    private final SkinTypeService service = Mockito.mock(SkinTypeService.class);
    private SkinTypeController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new SkinTypeController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((SkinTypeGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<SkinTypeGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<SkinTypeGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<SkinType>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((SkinType) response.getEntity()).getName());
