...
class TroubleFallingAsleepControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final TroubleFallingAsleep SAMPLE_ENTITY = new TroubleFallingAsleep(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final TroubleFallingAsleepPostRequest SAMPLE_ENTITY_DTO = new TroubleFallingAsleepPostRequest(NAME, CODE, true);

    private final TroubleFallingAsleepService service = Mockito.mock(TroubleFallingAsleepService.class);
    private TroubleFallingAsleepController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new TroubleFallingAsleepController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((TroubleFallingAsleepGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<TroubleFallingAsleepGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<TroubleFallingAsleepGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<TroubleFallingAsleep>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((TroubleFallingAsleep) response.getEntity()).getName());
