...
class EasternMedicineOpinionControllerTest {
    ...
    private static final EasternMedicineOpinion SAMPLE_ENTITY = new EasternMedicineOpinion(ID, NAME, CODE, true, SUBTITLE, LocalDateTime.now(), LocalDateTime.now());
    private static final EasternMedicineOpinionPostRequest SAMPLE_ENTITY_DTO = new EasternMedicineOpinionPostRequest(NAME, CODE, true, SUBTITLE);

    private final EasternMedicineOpinionService service = Mockito.mock(EasternMedicineOpinionService.class);
    private EasternMedicineOpinionController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new EasternMedicineOpinionController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((EasternMedicineOpinionGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<EasternMedicineOpinionGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<EasternMedicineOpinionGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<EasternMedicineOpinion>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((EasternMedicineOpinion) response.getEntity()).getName());
