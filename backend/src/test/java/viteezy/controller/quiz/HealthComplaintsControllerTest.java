...
class HealthComplaintsControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final HealthComplaints SAMPLE_ENTITY = new HealthComplaints(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final HealthComplaintsPostRequest SAMPLE_ENTITY_DTO = new HealthComplaintsPostRequest(NAME, CODE, true);

    private final HealthComplaintsService service = Mockito.mock(HealthComplaintsService.class);
    private HealthComplaintsController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new HealthComplaintsController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((HealthComplaintsGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<HealthComplaintsGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<HealthComplaintsGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<HealthComplaints>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((HealthComplaints) response.getEntity()).getName());
