...
import viteezy.controller.dto.quiz.SmokeGetResponse;
import viteezy.controller.dto.quiz.SmokePostRequest;
import viteezy.domain.quiz.Smoke;
import viteezy.service.quiz.SmokeService;
...
class SmokeControllerTest {
    ...
    private static final Smoke SAMPLE_ENTITY = new Smoke(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final SmokePostRequest SAMPLE_ENTITY_DTO = new SmokePostRequest(NAME, CODE, true);

    private final SmokeService service = Mockito.mock(SmokeService.class);
    private SmokeController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new SmokeController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((SmokeGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<SmokeGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<SmokeGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<Smoke>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((Smoke) response.getEntity()).getName());
