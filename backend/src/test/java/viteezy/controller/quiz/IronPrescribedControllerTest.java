...
class IronPrescribedControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final IronPrescribed SAMPLE_ENTITY = new IronPrescribed(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final IronPrescribedPostRequest SAMPLE_ENTITY_DTO = new IronPrescribedPostRequest(NAME, CODE, true);

    private final IronPrescribedService service = Mockito.mock(IronPrescribedService.class);
    private IronPrescribedController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new IronPrescribedController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((IronPrescribedGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<IronPrescribedGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<IronPrescribedGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<IronPrescribed>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((IronPrescribed) response.getEntity()).getName());
