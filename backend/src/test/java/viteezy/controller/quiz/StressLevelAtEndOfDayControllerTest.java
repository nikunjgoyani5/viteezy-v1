...
class StressLevelAtEndOfDayControllerTest {
    ...
    private static final StressLevelAtEndOfDay SAMPLE_ENTITY = new StressLevelAtEndOfDay(ID, NAME, CODE, true, SUBTITLE, LocalDateTime.now(), LocalDateTime.now());
    private static final StressLevelAtEndOfDayPostRequest SAMPLE_ENTITY_DTO = new StressLevelAtEndOfDayPostRequest(NAME, CODE, true, SUBTITLE);

    private final StressLevelAtEndOfDayService service = Mockito.mock(StressLevelAtEndOfDayService.class);
    private StressLevelAtEndOfDayController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new StressLevelAtEndOfDayController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((StressLevelAtEndOfDayGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<StressLevelAtEndOfDayGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<StressLevelAtEndOfDayGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<StressLevelAtEndOfDay>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((StressLevelAtEndOfDay) response.getEntity()).getName());
