...
class LoseWeightChallengeControllerTest {
    ...
    private static final LoseWeightChallenge SAMPLE_ENTITY = new LoseWeightChallenge(ID, NAME, CODE, true, SUBTITLE, LocalDateTime.now(), LocalDateTime.now());
    private static final LoseWeightChallengePostRequest SAMPLE_ENTITY_DTO = new LoseWeightChallengePostRequest(NAME, CODE, true, SUBTITLE);

    private final LoseWeightChallengeService service = Mockito.mock(LoseWeightChallengeService.class);
    private LoseWeightChallengeController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new LoseWeightChallengeController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((LoseWeightChallengeGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<LoseWeightChallengeGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<LoseWeightChallengeGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<LoseWeightChallenge>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((LoseWeightChallenge) response.getEntity()).getName());
