...
import viteezy.controller.dto.quiz.DietTypeGetResponse;
import viteezy.controller.dto.quiz.DietTypePostRequest;
import viteezy.domain.quiz.DietType;
import viteezy.service.quiz.DietTypeService;
...
class DietTypeControllerTest {
    ...
    private static final DietType SAMPLE_ENTITY = new DietType(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final DietTypePostRequest SAMPLE_ENTITY_DTO = new DietTypePostRequest(NAME, CODE, true);

    private final DietTypeService service = Mockito.mock(DietTypeService.class);
    private DietTypeController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new DietTypeController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((DietTypeGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<DietTypeGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<DietTypeGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<DietType>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((DietType) response.getEntity()).getName());
