...
class AmountOfProteinConsumptionControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final AmountOfProteinConsumption SAMPLE_ENTITY = new AmountOfProteinConsumption(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final AmountOfProteinConsumptionPostRequest SAMPLE_ENTITY_DTO = new AmountOfProteinConsumptionPostRequest(NAME, CODE, true);

    private final AmountOfProteinConsumptionService service = Mockito.mock(AmountOfProteinConsumptionService.class);
    private AmountOfProteinConsumptionController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new AmountOfProteinConsumptionController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((AmountOfProteinConsumptionGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<AmountOfProteinConsumptionGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<AmountOfProteinConsumptionGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<AmountOfProteinConsumption>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((AmountOfProteinConsumption) response.getEntity()).getName());
