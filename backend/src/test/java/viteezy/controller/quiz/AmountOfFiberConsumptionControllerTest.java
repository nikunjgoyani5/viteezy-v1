...
class AmountOfFiberConsumptionControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final AmountOfFiberConsumption SAMPLE_ENTITY = new AmountOfFiberConsumption(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final AmountOfFiberConsumptionPostRequest SAMPLE_ENTITY_DTO = new AmountOfFiberConsumptionPostRequest(NAME, CODE, true);

    private final AmountOfFiberConsumptionService service = Mockito.mock(AmountOfFiberConsumptionService.class);
    private AmountOfFiberConsumptionController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new AmountOfFiberConsumptionController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((AmountOfFiberConsumptionGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<AmountOfFiberConsumptionGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<AmountOfFiberConsumptionGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<AmountOfFiberConsumption>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((AmountOfFiberConsumption) response.getEntity()).getName());
