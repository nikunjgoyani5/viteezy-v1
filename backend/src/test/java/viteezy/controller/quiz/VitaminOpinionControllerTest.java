...
class VitaminOpinionControllerTest {
    ...
    private static final VitaminOpinion SAMPLE_ENTITY = new VitaminOpinion(ID, NAME, CODE, true, SUBTITLE, LocalDateTime.now(), LocalDateTime.now());
    private static final VitaminOpinionPostRequest SAMPLE_ENTITY_DTO = new VitaminOpinionPostRequest(NAME, CODE, true, SUBTITLE);

    private final VitaminOpinionService service = Mockito.mock(VitaminOpinionService.class);
    private VitaminOpinionController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new VitaminOpinionController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((VitaminOpinionGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<VitaminOpinionGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<VitaminOpinionGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<VitaminOpinion>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((VitaminOpinion) response.getEntity()).getName());
