...
class AllergyTypeAnswerControllerTest {
    ...
    private static final AllergyTypeAnswer SAMPLE_ENTITY = new AllergyTypeAnswer(ID, QUIZ_ID, ALLERGY_ID, LocalDateTime.now(), LocalDateTime.now());
    private static final CategorizedAnswer CATEGORIZED_ANSWER = new CategorizedAnswer(QUIZ_EXTERNAL_REFERENCE, ALLERGY_ID);

    private final AllergyTypeAnswerService service = Mockito.mock(AllergyTypeAnswerService.class);
    private AllergyTypeAnswerController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new AllergyTypeAnswerController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getId(), ((AllergyTypeAnswerGetResponse)response.getEntity()).getId());
    ...
    void findByQuizExternalReference() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(SAMPLE_ENTITY.getId(), ((List<AllergyTypeAnswerGetResponse>)response.getEntity()).get(0).getId());
    ...
    void findByQuizExternalReferenceEmptyList() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getByQuizExternalReference(QUIZ_EXTERNAL_REFERENCE);
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertTrue(((List<AllergyTypeAnswerGetResponse>) response.getEntity()).isEmpty());
        });
