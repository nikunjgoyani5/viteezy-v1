...
class ThirtyMinutesOfSunControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final ThirtyMinutesOfSun SAMPLE_ENTITY = new ThirtyMinutesOfSun(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final ThirtyMinutesOfSunPostRequest SAMPLE_ENTITY_DTO = new ThirtyMinutesOfSunPostRequest(NAME, CODE, true);

    private final ThirtyMinutesOfSunService service = Mockito.mock(ThirtyMinutesOfSunService.class);
    private ThirtyMinutesOfSunController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new ThirtyMinutesOfSunController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((ThirtyMinutesOfSunGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<ThirtyMinutesOfSunGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<ThirtyMinutesOfSunGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<ThirtyMinutesOfSun>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((ThirtyMinutesOfSun) response.getEntity()).getName());
