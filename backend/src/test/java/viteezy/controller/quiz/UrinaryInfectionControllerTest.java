...
class UrinaryInfectionControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final UrinaryInfection SAMPLE_ENTITY = new UrinaryInfection(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final UrinaryInfectionPostRequest SAMPLE_ENTITY_DTO = new UrinaryInfectionPostRequest(NAME, CODE, true);

    private final UrinaryInfectionService service = Mockito.mock(UrinaryInfectionService.class);
    private UrinaryInfectionController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new UrinaryInfectionController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((UrinaryInfectionGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<UrinaryInfectionGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<UrinaryInfectionGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<UrinaryInfection>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((UrinaryInfection) response.getEntity()).getName());
