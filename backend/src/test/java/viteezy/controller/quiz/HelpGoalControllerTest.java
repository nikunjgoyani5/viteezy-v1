...
import viteezy.controller.dto.quiz.HelpGoalGetResponse;
import viteezy.controller.dto.quiz.HelpGoalPostRequest;
import viteezy.domain.quiz.HelpGoal;
import viteezy.service.quiz.HelpGoalService;
...
class HelpGoalControllerTest {
    ...
    private static final HelpGoal SAMPLE_ENTITY = new HelpGoal(ID, NAME, CODE, true, SUBTITLE, LocalDateTime.now(), LocalDateTime.now());
    private static final HelpGoalPostRequest SAMPLE_ENTITY_DTO = new HelpGoalPostRequest(NAME, CODE, true, SUBTITLE);

    private final HelpGoalService service = Mockito.mock(HelpGoalService.class);
    private HelpGoalController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new HelpGoalController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((HelpGoalGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<HelpGoalGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<HelpGoalGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<HelpGoal>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((HelpGoal) response.getEntity()).getName());
