...
class AverageSleepingHoursControllerTest {

    private static final Long ID = 1L;
    private static final String NAME = "NAME";
    private static final String CODE = "123";
    private static final CloseException EXCEPTION = new CloseException("EXCEPTION", new Throwable());
    private static final AverageSleepingHours SAMPLE_ENTITY = new AverageSleepingHours(ID, NAME, CODE, true, LocalDateTime.now(), LocalDateTime.now());
    private static final AverageSleepingHoursPostRequest SAMPLE_ENTITY_DTO = new AverageSleepingHoursPostRequest(NAME, CODE, true);

    private final AverageSleepingHoursService service = Mockito.mock(AverageSleepingHoursService.class);
    private AverageSleepingHoursController controller;

    @BeforeEach
    void setUp() {
        Mockito.reset(service);
        controller = new AverageSleepingHoursController(service);
    }
    ...
    void find() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((AverageSleepingHoursGetResponse) response.getEntity()).getName());
    ...
    void findAll() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(1, ((List<AverageSleepingHoursGetResponse>) response.getEntity()).size());
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((List<AverageSleepingHoursGetResponse>) response.getEntity()).get(0).getName());
    ...
    void findAllEmpty() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            Response response = controller.getAll();
            Assertions.assertEquals(200, response.getStatus());
            Assertions.assertEquals(0, ((List<AverageSleepingHours>) response.getEntity()).size());
    ...
    void save() {
        ...
        Assertions.assertDoesNotThrow(() -> {
            ...
            Assertions.assertEquals(SAMPLE_ENTITY.getName(), ((AverageSleepingHours) response.getEntity()).getName());
