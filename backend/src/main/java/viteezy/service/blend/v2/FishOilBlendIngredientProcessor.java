...
public class FishOilBlendIngredientProcessor implements BlendIngredientProcessorV2 {
    ...
    private List<Optional<BlendIngredientReasonCode>> getReasonCode(QuizAggregatedInformationV2 quizAggregatedInformationV2) {
        ...
        if (isFishAllergy(quizAggregatedInformationV2.getAllergyTypes())) {
            return blendIngredientReasonCodes;
        }
        if (isVeganOrVegetarian(quizAggregatedInformationV2.getDietType())) {
            return blendIngredientReasonCodes;
        }
    ...
    private boolean isFishAllergy(List<AllergyType> allergyTypes) {
        return allergyTypes.stream().anyMatch(allergyType ->
                AllergyTypeCode.FISH.equals(allergyType.getCode())
                        || AllergyTypeCode.SHELLFISH.equals(allergyType.getCode()));
    }

    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isVeganOrVegetarian(Optional<DietType> dietTypeOptional) {
        return dietTypeOptional
                .map(dietType -> {
                    final String code = dietType.getCode();
                    return DietTypeCode.VEGAN.equals(code) || DietTypeCode.VEGETARIAN.equals(code);
                }).orElse(false);
    }
