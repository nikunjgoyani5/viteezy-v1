...
import viteezy.domain.quiz.code.UsageGoalCode;
...
public class GreenTeaExtractBlendIngredientProcessor implements BlendIngredientProcessorV2 {

    private static final BigDecimal FOUR_HUNDRED_FIFTY_UNITS = new BigDecimal("450.000");
    ...
    private BigDecimal getAmount(List<Optional<BlendIngredientReasonCode>> reasonCodeOptional) {
        return reasonCodeOptional.stream().findFirst()
                ...
                .map(code -> FOUR_HUNDRED_FIFTY_UNITS)
                .orElse(ZERO_UNITS);
    }
    ...
    private List<Optional<BlendIngredientReasonCode>> getReasonCode(QuizAggregatedInformationV2 quizAggregatedInformationV2) {
        ...
        if (isUsageGoalsWeightLoose(quizAggregatedInformationV2.getUsageGoals())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.GREEN_TEA_SUBJECT_LOOSE_WEIGHT));
            isGoal = true;
        }
        return blendIngredientReasonCodes;
    }
    ...
    private boolean isUsageGoalsWeightLoose(List<UsageGoal> usageGoalList) {
        return usageGoalList.stream().anyMatch(usageGoal ->
                UsageGoalCode.WEIGHT_LOOSE.equals(usageGoal.getCode()));
    }
}
