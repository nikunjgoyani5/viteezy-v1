...
public class DetoxFormulaBlendIngredientProcessor implements BlendIngredientProcessorV2 {

    private static final Logger LOGGER = LoggerFactory.getLogger(DetoxFormulaBlendIngredientProcessor.class);
    ...
    private static final Integer PRIORITY_UNIT = 8;
    ...
    private String getExplanation(
        ...
        reasonCodeOptional = reasonCodeOptional.stream().distinct().collect(Collectors.toList());
    ...
    private List<Optional<BlendIngredientReasonCode>> getReasonCode(QuizAggregatedInformationV2 quizAggregatedInformationV2) {
        ...
        if (isEatingMuchFish(quizAggregatedInformationV2.getAmountOfFishConsumption())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.DETOX_FORMULA_EATING_MUCH_FISH));
            PRIORITY_AMOUNT += 1;
        }
        if (isDrinkingDailySixAlcoholicDrinks(quizAggregatedInformationV2.getDailySixAlcoholicDrinks())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.DETOX_FORMULA_DRINKING));
            PRIORITY_AMOUNT += 1;
        }
        if (isDrinkingWeeklyTwelveAlcoholicDrinks(quizAggregatedInformationV2.getWeeklyTwelveAlcoholicDrinks())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.DETOX_FORMULA_DRINKING));
            PRIORITY_AMOUNT += 1;
        }
        if (isSmoking(quizAggregatedInformationV2.getSmoke())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.DETOX_SMOKE));
            PRIORITY_AMOUNT += 1;
        }
        return blendIngredientReasonCodes;
    }
    ...
    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isEatingMuchFish(Optional<AmountOfFishConsumption> amountOfFishConsumptionOptional) {
    ...
    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isDrinkingDailySixAlcoholicDrinks(Optional<DailySixAlcoholicDrinks> dailySixAlcoholicDrinksOptional) {
        return dailySixAlcoholicDrinksOptional
                .map(dailySixAlcoholicDrinks -> {
                    final String code = dailySixAlcoholicDrinks.getCode();
                    return AnswerCode.YES.equals(code);
                }).orElse(false);
    }

    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isDrinkingWeeklyTwelveAlcoholicDrinks(Optional<WeeklyTwelveAlcoholicDrinks> weeklyTwelveAlcoholicDrinksOptional) {
        return weeklyTwelveAlcoholicDrinksOptional
                .map(weeklyTwelveAlcoholicDrinks -> {
                    final String code = weeklyTwelveAlcoholicDrinks.getCode();
                    return AnswerCode.YES.equals(code);
                }).orElse(false);
    }

    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isSmoking(Optional<Smoke> smokeOptional) {
        return smokeOptional
                .map(smoke -> {
                    final String code = smoke.getCode();
                    return SmokeCode.SMOKING.equals(code);
                }).orElse(false);
    }
}
