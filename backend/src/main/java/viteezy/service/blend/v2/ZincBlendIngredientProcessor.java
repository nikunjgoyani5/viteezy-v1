...
public class ZincBlendIngredientProcessor implements BlendIngredientProcessorV2 {

    private static final BigDecimal TWO_HUNDRED_FOUR_UNITS = new BigDecimal("204.000");
    ...
    private static final Integer PRIORITY_UNIT = 5;
    ...
    private BigDecimal getAmount(List<Optional<BlendIngredientReasonCode>> reasonCodeOptional) {
        return reasonCodeOptional.stream().findFirst()
                ...
                .map(code -> TWO_HUNDRED_FOUR_UNITS)
                .orElse(ZERO_UNITS);
    }
    ...
    private List<Optional<BlendIngredientReasonCode>> getReasonCode(QuizAggregatedInformationV2 quizAggregatedInformationV2) {
        ...
        if (isChildrenWish(quizAggregatedInformationV2.getChildrenWish())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.ZINC_CHILD_WISH));
            PRIORITY_AMOUNT += 1;
        }
    ...
    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isChildrenWish(Optional<ChildrenWish> childrenWishOptional) {
        return childrenWishOptional
                .map(childrenWish -> {
                    final String code = childrenWish.getCode();
