...
public class GutSupportBlendIngredientProcessor implements BlendIngredientProcessorV2 {
    ...
    private List<Optional<BlendIngredientReasonCode>> getReasonCode(QuizAggregatedInformationV2 quizAggregatedInformationV2) {
        ...
        if (isUsageGoalDigestion(quizAggregatedInformationV2.getUsageGoals())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.GUT_SUPPORT_SUBJECT));
            isGoal = true;
            PRIORITY_AMOUNT += 1;
        }
        if (isDigestionAmountIrregular(quizAggregatedInformationV2.getDigestionAmount())) {
            PRIORITY_AMOUNT += 1;
        }
        if (isDigestionOccurrenceFlatulence(quizAggregatedInformationV2.getDigestionOccurrence())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.GUT_SUPPORT_FLATULENCE));
            PRIORITY_AMOUNT += 1;
        }
        if (isDigestionOccurrenceBloatedFeeling(quizAggregatedInformationV2.getDigestionOccurrence())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.GUT_SUPPORT_BLOATED_FEELING));
            PRIORITY_AMOUNT += 1;
        }
        if (isDigestionOccurrenceBurp(quizAggregatedInformationV2.getDigestionOccurrence())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.GUT_SUPPORT_BURP));
            PRIORITY_AMOUNT += 1;
        }
        if (isDigestionOccurrenceBadDigestion(quizAggregatedInformationV2.getDigestionOccurrence())) {
            blendIngredientReasonCodes.add(Optional.of(BlendIngredientReasonCode.GUT_SUPPORT_BAD_DIGESTION));
            PRIORITY_AMOUNT += 1;
        }
        return blendIngredientReasonCodes;
    }
    ...
    private boolean isUsageGoalDigestion(List<UsageGoal> usageGoalList) {
        return usageGoalList.stream().anyMatch(usageGoal ->
                UsageGoalCode.DIGESTION.equals(usageGoal.getCode()));
    }

    private boolean isDigestionAmountIrregular(Optional<DigestionAmount> digestionAmountOptional) {
        return digestionAmountOptional
                .map(digestionAmount -> {
                    final String code = digestionAmount.getCode();
                    return DigestionAmountCode.IRREGULAR.equals(code);
                }).orElse(false);
    }

    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isDigestionOccurrenceFlatulence(Optional<DigestionOccurrence> digestionOccurrenceOptional) {
        return digestionOccurrenceOptional
                .map(digestionOccurrence -> {
                    final String code = digestionOccurrence.getCode();
                    return DigestionOccurrenceCode.FLATULENCE.equals(code);
                }).orElse(false);
    }

    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isDigestionOccurrenceBloatedFeeling(Optional<DigestionOccurrence> digestionOccurrenceOptional) {
        return digestionOccurrenceOptional
                .map(digestionOccurrence -> {
                    final String code = digestionOccurrence.getCode();
                    return DigestionOccurrenceCode.BLOATED_FEELING.equals(code);
                }).orElse(false);
    }

    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isDigestionOccurrenceBurp(Optional<DigestionOccurrence> digestionOccurrenceOptional) {
        return digestionOccurrenceOptional
                .map(digestionOccurrence -> {
                    final String code = digestionOccurrence.getCode();
                    return DigestionOccurrenceCode.BURP.equals(code);
                }).orElse(false);
    }

    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    private boolean isDigestionOccurrenceBadDigestion(Optional<DigestionOccurrence> digestionOccurrenceOptional) {
        return digestionOccurrenceOptional
                .map(digestionOccurrence -> {
                    final String code = digestionOccurrence.getCode();
                    return DigestionOccurrenceCode.BAD_DIGESTION.equals(code);
                }).orElse(false);
    }
}
